...
public class UpdateUsernameUseCaseSync {
    ...
    public UseCaseResult updateUsernameSync(
            String userId,
            String username
    ) {
        EndpointResult endpointResult = null;
        try {
            endpointResult = mUpdateUsernameHttpEndpointSync.updateUsername(
                    userId,
                    username
            );
        ...
        if (isSuccessfulEndpointResult(endpointResult)) {
            ...
            mEventBusPoster.postEvent(
                    new UserDetailsChangedEvent(
                            new User(
                                    userId,
                                    username
                            )
